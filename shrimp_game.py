...
# ==============================================================================
# 1. DYNAMIC CONFIGURATION (TEAMS, DAYS & MODES)
# ==============================================================================
...
if not st.session_state.game_started:
    ...
    # Keuze tussen klassikaal (synchroon) of thuis oefenen (asynchroon)
    game_mode = st.radio(
        "Select Game Mode:",
    ...
    st.session_state.game_mode = "class" if "Live Class" in game_mode else "practice"
    
    # Pseudo-random Seed configuratie
    use_custom_seed = st.checkbox("Set specific Game Seed? (Ensures exact same demand sequence for replayability)", value=False)
    if use_custom_seed:
        game_seed = st.number_input("Enter integer Seed Code:", min_value=1, max_value=999999, value=42, step=1)
    else:
        game_seed = random.randint(1000, 9999)
        st.info(f"Generated Random Seed: **{game_seed}** (Share this if you want others to get the same demand at home)")

    total_days = st.number_input("Number of game rounds (days):", min_value=1, max_value=30, value=5, step=1)
    
    # In practice mode speelt een student vaak alleen
    default_teams = 3 if st.session_state.game_mode == "class" else 1
...
if "current_day_index" not in st.session_state:
    ...
    st.session_state.teams = {
        team: {"inventory": 0, "history": [], "finished": False} for team in st.session_state.team_names
    }
...
# Bepaal of we klaar zijn om de dag te berekenen op basis van de modus
if st.session_state.game_mode == "class":
    all_teams_submitted = set(st.session_state.team_names) == set(teams_submitted)
else:
    # In practice-mode mag er berekend worden zodra de *geselecteerde* actieve partij heeft ingestuurd
    all_teams_submitted = len(teams_submitted) > 0

# ==============================================================================
# 3. 👑 ADMIN PANEL & PARAMETERS (Aangepast met Seed-logica)
# ==============================================================================
st.sidebar.header("👑 Admin Control Panel")
st.sidebar.markdown(f"**Mode:** {st.session_state.game_mode.upper()} | **Seed:** `{st.session_state.game_seed}`")
...
if st.session_state.game_over:
    ...
    if not all_teams_submitted:
        if st.session_state.game_mode == "class":
            ...
            st.sidebar.info("⏳ Awaiting order submission from the dashboard below...")
    else:
        if not st.session_state.demand_calculated:
            # Knop gedrag verschilt per modus
            button_label = "🎲 Generate Demand & Calculate Results" if st.session_state.game_mode == "class" else "⚡ Process My Order Immediately"
            
            if st.sidebar.button(button_label) or st.session_state.game_mode == "practice":
                
                # --- VERBETERDE SEED LOGICA ---
                # Initialiseer een lokale generator met de hoofd-seed
                rng = random.Random(st.session_state.game_seed)
                # Genereer opeenvolgende demands tot en met de huidige dag-index
                demand = 0
                for _ in range(st.session_state.current_day_index + 1):
                    # We gebruiken een normale verdeling gebaseerd op de unieke state van deze seed
                    raw_demand = rng.normalvariate(demand_mean, demand_std)
                    demand = max(0, int(raw_demand))
                
                st.session_state.daily_demand[current_day] = demand
                
                # Bereken resultaten voor de teams die deze ronde hebben ingediend
    ...
    if st.session_state.demand_calculated:
        ...
        # Automatisch voorschieten in oefenmodus of handmatig door de docent in de klas
        if st.session_state.game_mode == "practice" or st.sidebar.button("⏭️ Advance to Next Day"):
...
if st.session_state.game_over:
    ...
    if all_games_history:
        ...
        st.download_button(
            ...
            data=csv_data,
            file_name=f"shrimp_game_seed_{st.session_state.game_seed}.csv",
            mime="text/csv"
        )
